      ...
      {!loading && sortedBookings.length === 0 ? (
        ...
        <div className="grid grid-cols-1 gap-8">
          {sortedBookings.map((booking, index) => {
            ...
            const price = booking.type === 'ROOM' ? room?.price ?? 0 : flight?.price ?? 0;
              ...
              >
                ...
                <div className="w-full md:w-32 h-32 flex-shrink-0 relative">
                  <div className="w-full h-full bg-slate-900 rounded-3xl flex items-center justify-center shadow-lg border-2 border-white ring-1 ring-slate-100">
                    {booking.type === 'ROOM' ? <Bed className="w-12 h-12 text-white" /> : <Plane className="w-12 h-12 text-white" />}
                  </div>
                </div>
                ...
                <div className="w-full md:w-auto flex md:flex-col items-center md:items-end gap-6 md:gap-2">
                  <div className="flex-1 md:flex-none">
                    ...
                    <h4 className="text-2xl font-black text-slate-900">${price + 45}</h4>
                  </div>
